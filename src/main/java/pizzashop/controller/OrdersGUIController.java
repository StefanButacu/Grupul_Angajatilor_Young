...
public class OrdersGUIController {
    ...
    private void initData() {
        ...
        payOrder.setOnAction(event -> {
            orderPaymentList = menuData.stream()
                    ...
                    .collect(Collectors.toList());
            double totalAmount = orderPaymentList.stream().mapToDouble(e -> e).sum();
            if (totalAmount == 0)
                return;
    ...
    public void initialize() {
        ...
        addToOrder.setOnAction(event -> {
            ReadOnlyObjectProperty a = orderTable.getSelectionModel().selectedItemProperty();
            MenuDataModel pizza = ((MenuDataModel) a.getValue());
            pizza.setQuantity(orderQuantity.getValue());
        });
